...
def create_client(session: Dict[str, Any], server_id: Optional[str] = None) -> AsyncOpenAI:
    """Create a DeepSeek client instance using registry."""
    from AI.core.registry import get_client
    client = get_client("deepseek", {"session": session, "server_id": server_id})
...
def get_model(session: Dict[str, Any], server_id: Optional[str] = None) -> str:
    ...
    from AI.core.registry import get_client
    client = get_client("deepseek", {"session": session, "server_id": server_id})
    return client.resolve_model(session, server_id, default_model="deepseek-chat")
